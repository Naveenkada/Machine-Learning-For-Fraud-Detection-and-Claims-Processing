...
# ML Capabilities
# Load the best model and its metrics  
def load_best_model_and_metrics():  
    # Load the best model  
...
@app.route('/')  
def best_model():  
    best_model, metrics = load_best_model_and_metrics()  
    # Ensure metrics is a dictionary and print its structure for debugging  
    print("Metrics structure:", metrics)  

    try:  
        # Assuming the metrics have a structure as dict of dicts:  
        best_model_name = max(metrics, key=lambda k: metrics[k]['accuracy'])  # Get name of the best model  
